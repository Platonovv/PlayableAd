...
namespace Project.UI.Common
{
	public sealed class TutorialNudge : MonoBehaviour
	{
		...
		[SerializeField]
		private float _idleSeconds = 2.5f;

		[SerializeField]
		private float _fadeDuration = 0.3f;

		[SerializeField]
		private float _arrowBobAmplitude = 18f;

		[SerializeField]
		private float _arrowBobSpeed = 4f;
		...
		private void OnTargetSelected(TargetSelectedSignal _)
		{
			MarkInteracted();
		}
		...
		private IEnumerator FadeTo(float target)
		{
			if (!(_group == null))
			{
				...
				while (elapsed < _fadeDuration)
				{
					...
					_group.alpha = Mathf.Lerp(start, target, i);
					yield return null;
				}
				_group.alpha = target;
