...
namespace Project.Gameplay.Flow.States
{
    ...
    public sealed class MovingToTargetState : IState<BattleFlowContext>
    {
        ...
        private async UniTask Run(BattleFlowContext ctx, CancellationToken ct)
        {
            ...
            ctx.Indicator.Show(ctx.Player.transform, view.Stop);
            await ctx.Player.MoveTo(view.Stop.position, ct);
            ctx.Indicator.Hide();

            switch (view.Kind)
            {
                case Project.Domain.UnitKind.Chest: _flow.GoChestOpen(); break;
                case Project.Domain.UnitKind.Enemy: _flow.GoAttack();    break;
                default:                            _flow.GoIdle();      break;
            }
        }
    }
}
