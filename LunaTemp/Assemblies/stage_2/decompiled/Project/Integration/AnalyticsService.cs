...
namespace Project.Integration
{
	public sealed class AnalyticsService : MonoBehaviour
	{
		...
		{
			Debug.Log("[analytics] " + ev);
		}

		private void OnStart(PlayableStartedSignal _)
		{
			Log("start");
		}

		private void OnTarget(TargetSelectedSignal s)
		{
			Log("interaction:" + s.TargetId.Value);
		}

		private void OnAttack(AttackResolvedSignal s)
		{
			Log("attack_ok:" + s.TargetId.Value);
		}

		private void OnFail(AttackFailedSignal s)
		{
			Log("attack_fail:" + s.TargetId.Value);
		}

		private void OnChest(ChestCollectedSignal s)
		{
			Log("chest:" + s.PowerGain);
		}

		private void OnWon(BattleWonSignal _)
		{
			Log("won");
		}
