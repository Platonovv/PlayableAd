...
namespace Project.UI.EndCard
{
    ...
    public sealed class EndCardPresenter : MonoBehaviour
    {
        ...
        [SerializeField] private string _winSubtitleFormat  = "DEFEATED {0} · POWER {1}";
        [SerializeField] private string _loseSubtitleFormat = "DEFEATED {0} · POWER {1}";
        ...
        private void Start()
        {
            ...
            if (_root.Level != null) _lastPower = _root.Level.Player.Power;
        }
        ...
        private void OnKill(AttackResolvedSignal _) => _kills++;
        private void OnFail(AttackFailedSignal _) => _fails++;
        private void OnPowerChanged(PlayerPowerChangedSignal s) => _lastPower = s.NewValue;

        private void OnWon(BattleWonSignal signal)
            => _view.Show(_winTitle, FormatSubtitle(_winSubtitleFormat), _root.Balance.EndCardDelay, showCta: true, starCount: ComputeWinStars());

        private void OnLost(BattleLostSignal signal)
            => _view.Show(_loseTitle, FormatSubtitle(_loseSubtitleFormat), _root.Balance.EndCardDelay, showCta: false, starCount: 0);

        private string FormatSubtitle(string fmt) => string.Format(fmt, _kills, _lastPower);

        private int ComputeWinStars()
        {
            if (_fails == 0) return 3;
            if (_fails <= 2) return 2;
            return 1;
        }
