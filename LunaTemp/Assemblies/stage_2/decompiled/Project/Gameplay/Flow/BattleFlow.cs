...
namespace Project.Gameplay.Flow
{
	public sealed class BattleFlow : MonoBehaviour
	{
		...
		[SerializeField]
		private FloatingNumber _floatingNumberPrefab;

		[SerializeField]
		private Transform _floatingNumbersRoot;

		[SerializeField]
		private Color _enemyArrowColor = new Color(0.95f, 0.25f, 0.25f, 1f);

		[SerializeField]
		private Color _winnableArrowColor = new Color(0.25f, 0.55f, 1f, 1f);

		[SerializeField]
		private Color _chestArrowColor = new Color(0.3f, 0.85f, 0.35f, 1f);
		...
		public void Init(BalanceConfig balance, SignalBus signals, PlayerView player, Battle battle, Dictionary<UnitId, UnitView> views)
		{
			Battle = battle;
			Pool<FloatingNumber> pool = null;
			if (_floatingNumberPrefab != null && _floatingNumbersRoot != null)
			{
				pool = new Pool<FloatingNumber>(_floatingNumberPrefab, _floatingNumbersRoot, 4);
			}
		...
		private void OnTargetPreview(TargetPreviewSignal signal)
		{
			...
			else if (view.Kind != 0 && !(_previewing == view))
			{
				if (_previewing != null)
				{
				...
				_previewing = view;
				bool isWarning = view.Kind == UnitKind.Enemy && _ctx.Battle.Player.Power < view.Unit.Power;
		...
		{
			if (_fsm.Current == _idle && _ctx.Views.TryGetValue(signal.TargetId, out var view) && view.Unit.IsAlive && view.Kind != 0)
			{
				if (_previewing != null)
				{
					_previewing.SetPreview(false);
		...
		internal void GoAttack()
		{
			_fsm.Set(_attack);
		}

		internal void GoChestOpen()
		{
			_fsm.Set(_chest);
		}
