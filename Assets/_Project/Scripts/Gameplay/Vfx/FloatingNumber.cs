...
namespace Project.Gameplay.Vfx
{
    ...
    public sealed class FloatingNumber : MonoBehaviour
    {
        ...
        [SerializeField] private Color _gainColor = new(0.55f, 1f, 0.55f);
        [SerializeField] private float _arcHeight = 1.6f;

        public async UniTask Play(string value, Vector3 worldPosition, float rise, float duration, Pool<FloatingNumber> pool, CancellationToken ct)
        {
        ...
        public async UniTask PlayFlying(string value, Vector3 from, Transform target, float duration, Pool<FloatingNumber> pool, CancellationToken ct)
        {
            ...
            while (elapsed < duration)
            {
                ...
                if (k > 0.7f) _group.alpha = 1f - (k - 0.7f) / 0.3f;
