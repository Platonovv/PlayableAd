...
В разработке использовался AI-ассистент как инструмент ускорения — это сознательный выбор для соблюдения сжатого срока (3 дня вместо недели). В описании вакансии умение работать с AI-инструментами указано как плюс, поэтому фиксирую границы использования явно.

**Где помог AI:**
- **Шаблонная математика и геометрия** — кривые ease, расчёт траекторий полёта меча из сундука (parabola + side-wobble), формулы подгонки `AspectRatioFitter`, перевод frustum'а камеры в world-units для BG-квада.
- **Помощь в составлении документации** — черновики формулировок этого README, описание оптимизаций, граф сцены (правки и финальные тексты — мои).
- **Поиск решений известных проблем** — обход бага Mecanim в Unity Playworks (research форумов и документации), CS2001 при пробелах в пути, тонкости рефлексии под `Luna.Unity.Playable`.
- **Ускорение рутины** — генерация editor-утилит (постпроцессоры импорта, FbxSlimmer, MaterialFixer), редактирование SceneBuilder под изменения иерархии.

**Что осталось за человеком:**
- **Стек и его обоснование** — выбор Unity 2022.3 LTS (vs Unity 6), Legacy Animation (vs Mecanim), Built-in RP против URP, отказ от TMP/DOTween/VContainer/ECS, Vorbis-параметры аудио, политика текстур и crunch-сжатия.
- **Boilerplate и core-инфраструктура** — generic `StateMachine<TContext>`, `SignalBus`, `Pool<T>`, обвязка корутин-твинов, MVP-lite в UI, схема state machine боя.
- **Оптимизация размера билда** — package set в `manifest.json`, IL2CPP/WebGL флаги, `link.xml`, профилирование размера через `BuildSizeAudit`, итерационная подгонка под бюджет ≤5 MB (итог: 2.5 MB).
- **Геймплей-дизайн** — последовательность фаз, балансировка цифр в `BalanceConfig`/`LevelConfig`, поведение TapInput (press-and-release с превью), правила раскраски стрелки.
- **Отладка и интеграция** — диагностика реальных проблем сборки в Playworks, тестирование на устройствах, фикс регрессий после изменений, работа с Luna SDK через рефлексию.
- **Все продуктовые решения** — что включить в MVP, как расставить приоритеты при ограниченном времени, какие polish-фичи добить, какие — оставить в бэклоге.

Подход: AI как «второй парой рук» для механической работы и быстрой проверки идей. Все решения, влияющие на качество билда и архитектуру, принимаются и проверяются мной — в том числе путём ручного code review всех сгенерированных кусков перед коммитом.
